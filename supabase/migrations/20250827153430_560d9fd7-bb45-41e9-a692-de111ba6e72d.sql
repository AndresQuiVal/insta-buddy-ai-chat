-- Crear función que obtiene stats filtrados por usuarios de Hower
CREATE OR REPLACE FUNCTION public.grok_get_stats_filtered_by_hower_users(p_instagram_user_id text, p_period text)
RETURNS TABLE(abiertas integer, seguimientos integer, agendados integer)
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
DECLARE
  hower_usernames text[];
  response_data jsonb;
BEGIN
  -- Llamar a la Edge Function para obtener los usernames válidos de Hower
  SELECT content INTO response_data
  FROM http((
    'POST',
    current_setting('app.settings.supabase_url') || '/functions/v1/get-hower-usernames',
    ARRAY[
      http_header('Authorization', 'Bearer ' || current_setting('app.settings.service_role_key')),
      http_header('Content-Type', 'application/json')
    ],
    'application/json',
    jsonb_build_object('instagram_user_id', p_instagram_user_id)::text
  ));

  -- Verificar si la respuesta fue exitosa
  IF response_data->>'success' != 'true' THEN
    RAISE EXCEPTION 'Error getting Hower usernames: %', response_data->>'error';
  END IF;

  -- Extraer los usernames del response
  SELECT ARRAY(
    SELECT jsonb_array_elements_text(response_data->'data'->'usernames')
  ) INTO hower_usernames;
  ...
  WITH prospect_counts AS (
    SELECT 
      ...
      COUNT(*) FILTER (WHERE p.last_message_from_prospect = true 
        ...
        AND p.username = ANY(hower_usernames)) as pending_count,
      ...
      COUNT(*) FILTER (WHERE p.last_owner_message_at IS NOT NULL 
        ...
        AND COALESCE(pts.is_completed, false) = false
        AND p.username = ANY(hower_usernames)) as followup_count
    FROM prospects p
