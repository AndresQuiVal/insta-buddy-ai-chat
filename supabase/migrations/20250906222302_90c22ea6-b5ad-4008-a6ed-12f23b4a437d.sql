-- Update function to count pending follow-ups instead of completed ones
...
BEGIN
  ...
  WITH prospect_counts AS (
    SELECT 
      ...
      -- SEGUIMIENTOS: Prospectos que necesitan seguimiento (pendientes, no ya hechos)
      -- Que hayan respondido, pero que no les he contestado en más de 24 horas (o nunca)
      -- Y NO están tachados Y están en la lista de Hower
      COALESCE(COUNT(*) FILTER (WHERE p.last_message_from_prospect = true
        AND (
          p.last_owner_message_at IS NULL OR  -- Nunca les he enviado mensaje
          p.last_owner_message_at <= (now() - interval '24 hours')  -- Más de 24h desde mi último mensaje
        )
        AND COALESCE(pts.is_completed, false) = false  -- NO tachados
        AND (
          p.username = ANY(p_hower_usernames) OR 
