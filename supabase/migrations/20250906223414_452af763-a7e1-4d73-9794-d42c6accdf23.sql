-- Fix WhatsApp notification count to match UI logic
-- Only count prospects that are actually shown in "Prospectos a recontactar" section
...
BEGIN
  ...
  WITH prospect_counts AS (
    SELECT 
      ...
      -- SEGUIMIENTOS: EXACTAMENTE los mismos prospectos que muestra la UI en "Prospectos a recontactar"
      -- Solo los que han respondido + hace >= 1 día sin respuesta + estado apropiado + no tachados + en Hower
      COALESCE(COUNT(*) FILTER (WHERE p.last_message_from_prospect = true
        AND p.last_owner_message_at IS NOT NULL  -- SÍ les he enviado mensaje antes
