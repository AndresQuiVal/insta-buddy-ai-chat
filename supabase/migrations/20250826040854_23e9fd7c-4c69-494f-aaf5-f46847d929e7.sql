-- Corregir la función grok_get_stats para manejar correctamente los seguimientos
-- según la lógica del usuario: prospectos pasan a seguimiento después de 1 día 
-- desde el último mensaje del dueño, y NO deben aparecer tachados
...
BEGIN
  ...
  WITH prospect_counts AS (
    SELECT 
      ...
      -- O menos de 24h desde último mensaje del dueño, y NO tachados
      COUNT(*) FILTER (WHERE p.last_message_from_prospect = true 
        ...
        AND COALESCE(pts.is_completed, false) = false) as pending_count,
      
      -- SEGUIMIENTOS: prospectos contactados hace más de 1 día (24 horas) y NO tachados
      -- Estos NO deben estar tachados porque les vamos a enviar seguimiento
      COUNT(*) FILTER (WHERE p.last_owner_message_at IS NOT NULL 
        AND p.last_owner_message_at <= (now() - interval '24 hours')
        AND COALESCE(pts.is_completed, false) = false) as followup_count
    FROM prospects p
